"""Plain-text extraction (#60 / Decision #007).
...
MAX_TXT_BYTES = 50 * 1024 * 1024  # 50 MiB — align with PDF cap
...
def extract_txt(path: Path | str) -> ExtractResult:
    """
    Read a ``.txt`` file into a single FTS segment (page=1).

    Encoding: utf-8-sig → utf-8 → cp1252 → latin-1 (replace). Soft-fails on
    missing/oversize/unreadable files.
    """
    resolved = Path(path)
    result = ExtractResult(parser="stdlib-txt", page_count=1)
    ...
    if size > MAX_TXT_BYTES:
        result.status = "error"
        result.warnings.append(f"file exceeds {MAX_TXT_BYTES} bytes; skipped extract")
        result.page_count = 0
        return result
