...
### 07/14/2026 â Electron main / preload / renderer

What I learned: Electron has three layers that matter for talking to FastAPI. The main process owns the window and can `net.fetch` localhost. Preload uses `contextBridge` to expose a tiny safe API. The renderer only displays results â it should not get Node/filesystem access. Also: `loadFile` on a `.py` file is not an API call; uvicorn must be running and you hit `http://127.0.0.1:8000/`.

Why it matters: This is the pattern we reuse with React in `frontend/` â same main/preload bridge; React only renders and invokes IPC, never talks to FastAPI directly.

### 07/14/2026 â React through Electron only

What I learned: For the architecture spike, âdoes React work?â is the wrong test. The right test is React â Electron â FastAPI â Electron â React. A renderer `fetch` to localhost would skip the gatekeeper we need for search / query / ask-ai later.

Why it matters: One IPC shape (`checkHealth` today) stays stable while FastAPI endpoints change.

### 07/14/2026 â System capability detection
...
### 07/15/2026 â Packaging vs backend lifecycle
...
### 07/15/2026 â Board audit vs duplicate issues

What I learned: Early planning created parallel issues for the same product rule (e.g. opt-in folders as both #40 and #97) and left finished milestones open. A short audit pass â close empty milestones, fold duplicates, park carry-overs on a bridge milestone (v0.1.1), link nice-to-haves from `ideas.md` â makes "what next" obvious without deleting roadmap ambition.

Why it matters: Stops thrashing on Phase 3 noise and keeps #96 from floating without a home after leaving v0.1.0.

### 07/22/2026 â Owned uvicorn without --reload
...
### 07/24/2026 — Classic-first routing stub vs hybrid

What I learned: A hybrid *routing stub* (#98) is not hybrid *search*. Shipping named classic/semantic/LLM stages with only classic live keeps Decision #002 honest without pulling embeddings into v0.3. Empty-result suggestions/fuzzy are a separate classic-IR idea (ideas.md), not Ollama and not the stub.
